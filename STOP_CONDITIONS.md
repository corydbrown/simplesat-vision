...
2. **Leave a `// STOP — <reason>` comment** in the code at the line where you got stuck (or, if not line-specific, at the top of the file you were editing).
3. **Push the branch** so the supervisor's `/sweep` can find the STOP marker on its diff scan.
...
## Soft stops — push your WIP with a STOP marker before continuing
...
These don't require a STOP marker, but should appear in the PR's "Things worth pushing on" section so the reviewer sees them:
...
- **Silent worker stalls** (SVP-71 + SVP-74 committed locally then stalled ~3 hours before pushing). Soft stop "15 min blocked" + STOP marker on push = immediate signal once the supervisor sweeps.
...
**The in-code `// STOP — <reason>` comment is the escalation channel.** Workers leave it at the line where they got stuck (or top-of-file if not line-specific), commit, and push. The supervisor's `/sweep` skill scans pushed-branch diffs for `STOP —` markers and surfaces them ahead of any PR review.

Slack is **not** used as a coordination surface in this project — don't try to ping anyone there. The STOP marker + supervisor sweep is the loop.
