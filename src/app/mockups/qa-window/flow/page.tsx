...
/**
 * Round-5 polished candidate. Two-column always-sticky layout, Inspect
 * sidebar manages everything (citations, comments, reactions), multi-Esc
 * unwinding, Slack-style reactions with curated 6-emoji set, citation
 * chips as `category · 4/5` (no colored circle), full keyboard wiring
 * with the `keys` T → score-picker bug fixed.
 */
  ...
  for (const cat of sampleTicket.evaluation.categories) {
    for (const msgId of cat.highlightedMessageIds) {
      out.push({
        ...
        messageId: msgId,
        categoryId: cat.id,
        // AI seeds each citation with the category's own AI score so chips
        // render `4/5` rather than blank from the first paint.
        score: cat.aiScore,
        aiSuggested: true,
      });
    }
...
export default function FlowMockupPage() {
  ...
  const openInspect = useCallback(
    (id: string) => {
      ...
      closeAllOverlays();
      setPendingComment("");
      // Auto-focus comment input on Inspect open.
  ...
  useEffect(() => {
    function handler(e: globalThis.KeyboardEvent) {
      ...
      if (e.key === "Escape") {
        ...
        if (isTyping) {
          // Blur composer but stay in Inspect.
          e.preventDefault();
      ...
      // Composer captures the rest of its keys (incl Cmd+Enter).
      if (isTyping) return;
    ...
    // addCitation/toggleReaction/setCitationScore use only state setters
    // (stable identities); excluding them keeps the handler from
  ...
  // selected in the sidebar (critical for long convos).
  useEffect(() => {
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="relative pb-16">
        {/* Two-column ALWAYS sticky layout. Convo scrolls in the page;
            sidebar is sticky inside its own column. The grid never
            collapses or overlaps regardless of selection state. */}
        <div className="grid grid-cols-[1fr_380px] gap-6">
          ...
          {/* Sticky right column — always present, never overlays. */}
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-dashed border-border bg-card/40 px-3 py-2">
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>
          Click a message to inspect. Use{" "}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Citation chips + comment count + reaction chips */}
        ...
        {/* Inline picker for keyboard-driven category / score flow */}
...
function InspectPanel({
  ...
  return (
    <div className="flex max-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-xl border border-border bg-card/95 shadow-xl backdrop-blur-md animate-in fade-in slide-in-from-right-2 duration-200">
      {/* Back arrow + Esc hint — NO CTA strip, NO score list, NO overall score */}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="space-y-5 p-4">
          ...
          {comments.length > 0 && (
            <section>
              <SectionHeader label="Comments" hint={`${comments.length}`} />
          ...
          {/* Composer — always present; auto-focused on Inspect open. */}
          <section>
  ...
  return (
    <li className="flex gap-2.5">
      ...
      <div className="min-w-0 flex-1 text-left">
        ...
        {/* Body is ALWAYS left-justified, regardless of who authored the
            original message — comments are reviewer notes, not chat. */}
