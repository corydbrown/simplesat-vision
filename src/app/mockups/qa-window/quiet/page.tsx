...
/**
 * Round-7 "quiet" — same comprehensive refinement of `tight` as sibling
 * `clean`, but with a different kbd-hint reveal mechanism: a "Show shortcuts"
 * Switch in the page header (not hold-Cmd). Default OFF — all kbd badges
 * fade hidden across the page (shortcuts still work). Toggle ON and the badges
 * fade in (~150ms).
 *
 * Round-7 vs round-6 "tight":
 *  - Slack-style icon-strip popup floats top-right of bubble, slightly
 *    overlapping; opaque bg-popover; icon-only by default; kbd badges appear
 *    below icons only when hints are revealed.
 *  - Citation chips use a 5-dot scale (●●●●○) instead of "4/5".
 *  - Arrow nav between columns AND within Inspect rows.
 *  - First Inspect row receives focus on open (NOT the composer).
 *  - Reactions on messages AND comments. ReactionAddButton inside Inspect
 *    expands LEFT (into the sidebar) so it never escapes the column.
 *  - Click outside both columns closes Inspect.
 *  - One menu on React popup click (fixed the duplicate-tray bug).
 *  - Bottom convo padding so the last message has breathing room.
 *  - Activity toggle, multi-Esc, category filter, "?" cheat sheet preserved.
 */
...
/** A focusable row inside the Inspect panel. */
type InspectRow =
  | { kind: "add" }
...
/** Render a kbd badge only when hints are revealed. Fade in on appear. */
function HintBadge({
  children,
  className,
}: {
  ...
  return (
    <span
      className={cn(
        "inline-flex animate-in fade-in duration-150",
        className,
      )}
    >
      <Kbd>{children}</Kbd>
...
/** Render an inline cluster (often `· Esc`) only when hints are revealed. */
function HintInline({
  children,
  className,
}: {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 animate-in fade-in duration-150",
        className,
      )}
    >
      {children}
...
const INITIAL_MSG_REACTIONS: Record<string, ReactionState> = {
...
export default function QuietMockupPage() {
  ...
  const [inspectZone, setInspectZone] = useState<"convo" | "inspect">("convo");
  const [inspectRowIndex, setInspectRowIndex] = useState(0);
  ...
  const [reactionsByMsg, setReactionsByMsg] =
    useState<Record<string, ReactionState>>(INITIAL_MSG_REACTIONS);
  ...
  /* ---- Inspect row computation ---- */

  const inspectRows = useMemo<InspectRow[]>(() => {
    if (!inspectMessage) return [];
    const cits = citationsByMessage.get(inspectMessage.id) ?? [];
    const cmts = commentsByMsg[inspectMessage.id] ?? [];
    const isAgent = inspectMessage.role === "agent";
    const availableCats = evaluation.categories.filter(
      (c) => !cits.some((cit) => cit.categoryId === c.id),
    );
    const rows: InspectRow[] = [];
    if (isAgent) {
      for (const c of cits) {
        rows.push({ kind: "citation", categoryId: c.categoryId });
      }
    ...
    for (const c of cmts) {
      rows.push({ kind: "comment", id: c.id });
    }
    return rows;
  }, [inspectMessage, citationsByMessage, commentsByMsg, evaluation.categories]);

  const focusedInspectRow: InspectRow | null = useMemo(() => {
    if (inspectRows.length === 0) return null;
    const i = Math.max(0, Math.min(inspectRows.length - 1, inspectRowIndex));
    return inspectRows[i];
  }, [inspectRows, inspectRowIndex]);

  /* ---- Mutators ---- */

  const openInspect = useCallback(
    (id: string, opts?: { addCitation?: boolean; focus?: "composer" }) => {
      setInspectId(id);
      setFocusedId(id);
      ...
      // Zone always starts in inspect (per brief: "Focus lands on the first
      // nav item"). RowIndex defaults to 0 = first row.
      setInspectZone("inspect");
      setInspectRowIndex(0);
      // Composer focus is OPT-IN only (e.g. when user clicked the comment
      // chip on a bubble). Brief: NO auto-focus on Inspect open by default.
      if (opts?.focus === "composer") {
  ...
  function toggleMsgReaction(messageId: string, emoji: Reaction) {
    ...
    function handler(e: globalThis.KeyboardEvent) {
      ...
      if (reactionTrayMessageId) {
        ...
        if (Number.isInteger(n) && n >= 1 && n <= REACTIONS.length) {
          e.preventDefault();
          toggleMsgReaction(reactionTrayMessageId, REACTIONS[n - 1]);
          setReactionTrayMessageId(null);
        }
        return;
      }
      ...
      /* Zone-aware nav */
      const inInspect = inspectId !== null && inspectZone === "inspect";

      if (inInspect) {
        // Within Inspect: Up/Down row nav, Left → back to convo, Enter → activate row, numeric → score focused citation.
        switch (e.key) {
          case "ArrowDown":
          case "j":
          case "J":
            e.preventDefault();
            setInspectRowIndex((i) =>
              Math.min(inspectRows.length - 1, i + 1),
            );
            return;
          case "ArrowUp":
          case "k":
          case "K":
            e.preventDefault();
            setInspectRowIndex((i) => Math.max(0, i - 1));
            return;
          case "ArrowLeft":
            e.preventDefault();
            setInspectZone("convo");
          ...
          case "Enter": {
            ...
            // citation: already focused; reasoning + editor visible inline.
            // comment: no-op.
            return;
          }
        }

        // Numeric 1-5 sets score on focused (non-binary) citation.
        if (focusedInspectRow?.kind === "citation" && inspectId) {
          const n = Number(e.key);
          if (Number.isInteger(n) && n >= 1 && n <= 5) {
            const cat = evaluation.categories.find(
              (c) => c.id === focusedInspectRow.categoryId,
            );
            if (cat && cat.scaleType !== "binary") {
              e.preventDefault();
              setCitationScore(inspectId, focusedInspectRow.categoryId, n);
              return;
            }
          }
        }
      ...
      if (e.key === "c" || e.key === "C") {
        e.preventDefault();
        openInspect(focusedId, { focus: "composer" });
        return;
      }
      if (e.key === "t" || e.key === "T") {
  ...
  // Smooth-scroll focused convo bubble into view.
  useEffect(() => {
    if (!focusedId) return;
  ...
  // Auto-scroll first cited message to top when a category is selected.
  useEffect(() => {
  ...
  // Note: inspectRowIndex is clamped at read time in focusedInspectRow,
  // so no setState-in-effect is needed when rows shape changes.
    ...
    <HintsContext.Provider value={showHints}>
      <TooltipProvider delayDuration={200}>
        <div className="relative pb-12">
          ...
          <div className="grid grid-cols-[1fr_380px] gap-6" ref={layoutRef}>
            <div className="min-w-0">
              ...
              <div className="mt-4 space-y-3 pb-[150px]">
                {messages.map((msg) => {
                  ...
                  const isFocused =
                    focusedId === msg.id &&
                    (inspectId === null || inspectZone === "convo");
                  ...
                  return (
                    <div key={msg.id}>
                      <MessageBubble
                        ...
                        onClickComment={() =>
                          openInspect(msg.id, { focus: "composer" })
                        }
                        onClickCite={() => {
                        ...
                        onClickReact={() => {
                          // ONE menu only — open the inline tray; do not also
                          // bubble up to anything else.
                          setReactionTrayMessageId(msg.id);
                        }}
                        onClickInspect={() => openInspect(msg.id)}
                        onClickCitationChip={(catId) => {
                          openInspect(msg.id);
                          // Focus that citation row in Inspect.
                          requestAnimationFrame(() => {
                            const cits =
                              citationsByMessage.get(msg.id) ?? [];
                            const idx = cits.findIndex(
                              (c) => c.categoryId === catId,
                            );
                            if (idx >= 0) {
                              setInspectZone("inspect");
                              setInspectRowIndex(idx);
                            }
                          });
                        }}
                ...
                <EndOfConvoMarker dimmed={inspectId !== null} />
              </div>
            </div>
                ...
                {inspectMessage ? (
                  <InspectPanel
                    ...
                    onComposerFocus={() => {
                      ...
                      const idx = inspectRows.findIndex(
                        (r) => r.kind === "composer",
                      );
                      if (idx >= 0) {
                        setInspectZone("inspect");
...
/* -------------------- Page header (variant: top-of-page toggle) -------------------- */
...
function PageHeader({
  ...
  return (
    <div className="mb-4 flex items-center justify-between border-b border-border/60 pb-3 text-sm text-muted-foreground">
      <div className="flex items-center gap-2">
        <span className="font-medium text-foreground">QA window</span>
      ...
      <div className="flex items-center gap-4">
        <button
          type="button"
          onClick={onOpenCheat}
          className="flex cursor-pointer items-center gap-1.5 rounded-md px-1.5 py-1 transition-colors hover:bg-accent hover:text-foreground"
        >
        ...
        <label className="flex shrink-0 cursor-pointer items-center gap-2">
          ...
          <Switch checked={showHints} onCheckedChange={onToggleHints} />
        </label>
      </div>
    </div>
  );
...
function EndOfConvoMarker({ dimmed }: { dimmed: boolean }) {
  return (
    <div
      className={cn(
        "pt-4 text-center text-sm text-muted-foreground/60 transition-opacity duration-200",
        dimmed && "opacity-30",
      )}
      aria-hidden
    >
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Slack-style icon strip: floats top-right of the bubble,
              slightly overlapping the top edge. Opaque bg-popover. */}
          {popupVisible && (
            <BubblePopup
        ...
        {/* Inline reaction tray (keyboard `R` flow when no reactions yet). */}
...
function BubblePopup({
  ...
  return (
    <div
      // Float top-right of bubble, slightly overlapping the top edge.
      // Opaque bg-popover + shadow + small border-radius prevents bleed.
      className={cn(
        "absolute -top-3 right-2 z-20 flex items-stretch gap-0.5 rounded-md border border-border bg-popover p-0.5 shadow-md",
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          ...
          className={cn(
            "flex flex-col items-center justify-center rounded-sm px-1.5 py-1 transition-colors",
            disabled
  ...
  return (
    ...
    >
      ...
      {isBinary ? (
        ...
        <DotScale value={effectiveScore} hue={hue} size="sm" />
      )}
      ...
      {REACTIONS.map((emoji, idx) => {
        ...
        return (
          <button
            ...
            aria-label={`React with ${emoji}`}
            className={cn(
              "flex cursor-pointer flex-col items-center justify-center rounded-md transition-all hover:scale-110 hover:bg-accent",
              show ? "size-9" : "size-8",
              youReacted && "bg-primary/10",
            )}
          >
...
function InspectPanel({
  ...
  onComposerBlur: () => void;
  onClickRow: (row: InspectRow) => void;
  ...
  const focusedRowKey = focusedRow ? rowKey(focusedRow) : null;
  const inInspectZone = zone === "inspect";
  ...
  return (
    <div className="flex max-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-xl border border-border bg-card/95 shadow-xl backdrop-blur-md animate-in fade-in slide-in-from-right-2 duration-200">
      {/* Header: ← Back · Esc (Esc only when hints on); Message N right-justified. */}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="space-y-5 p-4">
                  ...
                  {citations.map((c) => {
                    ...
                    return (
                      <InspectCitationRow
                        ...
                        focused={inInspectZone && focusedRowKey === k}
                        onFocus={() =>
                          onClickRow({
                            kind: "citation",
                  ...
                  {addingCategory && pendingCategory ? (
                    ...
                    availableCategories.length > 0 && (
                      <button
                        ...
                        className={cn(
                          "flex w-full cursor-pointer items-center gap-2 rounded-lg border border-dashed px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-accent/40 hover:text-foreground",
                          inInspectZone && focusedRowKey === "add"
          ...
          {/* Composer — always present. Visually highlighted when focused via kbd nav. */}
          <section>
            <CommentComposer
              ref={composerRef}
              ...
              focused={inInspectZone && focusedRowKey === "composer"}
              onClickWrap={() => onClickRow({ kind: "composer" })}
            />
          </section>
                ...
                {comments.map((c) => {
                  ...
                  return (
                    <CommentRow
                      ...
                      focused={inInspectZone && focusedRowKey === k}
                      onFocus={() =>
                        onClickRow({ kind: "comment", id: c.id })
                      }
                      onToggleReaction={(emoji) =>
    ...
    >
      ...
      >
        ...
        {isBinary ? (
          ...
          <DotScale value={effectiveScore} hue={hue} size="md" />
        )}
        {citation.aiSuggested && (
          <Sparkles
            ...
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  ...
                  className={cn(
                    "flex size-8 cursor-pointer flex-col items-center justify-center gap-0.5 rounded-md border text-sm font-medium tabular-nums transition-all hover:scale-105",
                    n === effectiveScore
              ...
              <HintInline className="ml-2 text-xs text-muted-foreground">
                <Kbd>1</Kbd>
                <span>…</span>
                <Kbd>5</Kbd>
              </HintInline>
  ...
  return (
    <div className="rounded-lg border border-dashed border-border bg-background/40 p-2">
      <div className="mb-1 flex items-center justify-between px-1 text-sm text-muted-foreground">
        <span>
          Pick a category
          {show && ` — press 1–${Math.max(categories.length, 1)}`}
        </span>
        <button
          type="button"
          onClick={onCancel}
...
function CommentRow({
  ...
  return (
    <li
      onClick={onFocus}
      className={cn(
        "flex cursor-pointer gap-2.5 rounded-md p-1.5 transition-colors",
        focused ? "bg-accent/30 ring-1 ring-primary/30" : "hover:bg-accent/20",
      )}
    >
      <div
        ...
        {(hasReactions || trayOpen) && (
          <div className="mt-1.5 flex flex-wrap items-center gap-1">
            ...
            <ReactionAddButton
              ...
              reactions={reactions}
              // Inside the right-column sidebar — expand LEFT so the picker
              // never escapes the column.
              side="left"
              align="start"
            />
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="mt-1.5 flex items-center justify-end">
        ...
        >
          <span>Submit</span>
          {/* ⌘⏎ is ALWAYS visible — the Submit button is an exception to
              the hints-toggle rule per the brief. */}
  ...
  const groups: { title: string; rows: { keys: string[]; desc: string }[] }[] = [
    ...
    },
    {
      ...
      rows: [
        ...
        { keys: ["←"], desc: "Back to conversation" },
        { keys: ["↵"], desc: "Activate focused row" },
